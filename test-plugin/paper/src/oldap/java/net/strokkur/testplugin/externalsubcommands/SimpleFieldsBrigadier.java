...
import net.strokkur.testplugin.externalsubcommands.ExternalSimpleSubBlueprint;
 ...
 * {@link SimpleFields} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class SimpleFieldsBrigadier {
  public static final String NAME = "simplefields";
   ...
   * in {@link SimpleFields}. You can either retrieve the unregistered node with this method
   * or register it directly with {@link #register(Commands)}.
   */
  public static LiteralCommandNode<CommandSourceStack> create() {
    final SimpleFields instance = new SimpleFields();
    final ExternalSimpleSubBlueprint instanceDefaultSub = new ExternalSimpleSubBlueprint();
    final ExternalSimpleSubBlueprint instanceBalance = instance.balance;
    final ExternalSimpleSubBlueprint instanceHello = instance.hello;
    final ExternalSimpleSubBlueprint instanceWeather = instance.weather;

    return Commands.literal(NAME)
      .then(Commands.literal("default")
        .executes(ctx -> {
          instanceDefaultSub.execute(ctx.getSource().getSender());
          return Command.SINGLE_SUCCESS;
        })
      )
      .then(Commands.literal("balance")
        .executes(ctx -> {
          instanceBalance.execute(ctx.getSource().getSender());
          return Command.SINGLE_SUCCESS;
        })
      )
      .then(Commands.literal("hello")
        .executes(ctx -> {
          instanceHello.execute(ctx.getSource().getSender());
          return Command.SINGLE_SUCCESS;
        })
      )
      .then(Commands.literal("weather")
        .executes(ctx -> {
          instanceWeather.execute(ctx.getSource().getSender());
          return Command.SINGLE_SUCCESS;
        })
      )
      .build();
   ...
   */
  private SimpleFieldsBrigadier() throws IllegalAccessException {
