 ...
 * {@link FillBlockCommand} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class FillBlockCommandBrigadier {
  public static final String NAME = "fillblock";
   ...
   *     FillBlockCommandBrigadier.register(commands);
   * }
   * }</pre>
   */
   ...
   * in {@link FillBlockCommand}. You can either retrieve the unregistered node with this method
      ...
      .then(Commands.argument("pos1", ArgumentTypes.blockPosition())
        .then(Commands.argument("pos2", ArgumentTypes.blockPosition())
          .then(Commands.argument("state", ArgumentTypes.blockState())
            .executes(ctx -> {
              final FillBlockCommand executorClass = new FillBlockCommand(
                ctx.getArgument("pos1", BlockPositionResolver.class).resolve(ctx.getSource()),
                ctx.getArgument("pos2", BlockPositionResolver.class).resolve(ctx.getSource()),
                ctx.getArgument("state", BlockState.class)
              );
   ...
   */
  private FillBlockCommandBrigadier() throws IllegalAccessException {
