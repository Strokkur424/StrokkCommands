 ...
 * {@link RegistryArgumentsCommand} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class RegistryArgumentsCommandBrigadier {
  public static final String NAME = "registryarg";
   ...
   * in {@link RegistryArgumentsCommand}. You can either retrieve the unregistered node with this method
   * or register it directly with {@link #register(Commands)}.
   */
  public static LiteralCommandNode<CommandSourceStack> create() {
    final RegistryArgumentsCommand instance = new RegistryArgumentsCommand();

    return Commands.literal(NAME)
      .then(Commands.literal("play-sound")
        .then(Commands.argument("sound", ArgumentTypes.resource(RegistryKey.SOUND_EVENT))
          .executes(ctx -> {
            instance.execute(
              ctx.getSource().getSender(),
              ctx.getArgument("sound", Sound.class)
            );
            return Command.SINGLE_SUCCESS;
          })
        )
      )
      .then(Commands.literal("itemtype")
        ...
        .then(Commands.argument("type", ArgumentTypes.resource(RegistryKey.ITEM))
          .executes(ctx -> {
            ...
            instance.execute(
              ctx.getSource().getSender(),
              executor,
              ctx.getArgument("type", ItemType.class)
            );
            return Command.SINGLE_SUCCESS;
          })
      ...
      .then(Commands.literal("cow-variant")
        .then(Commands.argument("cowVariant", ArgumentTypes.resource(RegistryKey.COW_VARIANT))
          .executes(ctx -> {
            instance.execute(
              ctx.getSource().getSender(),
              ctx.getArgument("cowVariant", Cow.Variant.class)
            );
            return Command.SINGLE_SUCCESS;
          })
        )
      )
      .then(Commands.literal("memory-key")
        .then(Commands.argument("memoryKey", ArgumentTypes.resource(RegistryKey.MEMORY_MODULE_TYPE))
          .executes(ctx -> {
            instance.execute(
              ctx.getSource().getSender(),
              ctx.getArgument("memoryKey", MemoryKey.class)
            );
            return Command.SINGLE_SUCCESS;
          })
        )
   ...
   */
  private RegistryArgumentsCommandBrigadier() throws IllegalAccessException {
