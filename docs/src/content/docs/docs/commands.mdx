...
from calling `ctx.getSource().getCommandSender()`.

### Player executors
If your command requires a player to execute it, you have to choices to make:
1. You can either just `instanceof`-cast the sender:
...
The reason is that instead of casting the command sender, you get the `Player` entity as with the command
was executed as. This distinction exists because if the `/execute as` command. Using the `/executes` command,
you can change the player for which the command is executed. All Vanilla commands use the executor
instead of the sender for this exact reasons, so for parity reasons, you should do as well.

If the command was ran by a `Player`, `player == sender` will evaluate to `true`, `false`, if not.

## Adding a literal (subcommand)
You can declare top-level literals by adding a parameter to the `@Executes` annotation.
...
The reason for this is so that the annotation processor can run and generated the required source files for your command.

The source file will always be generated in the same package as the command declaration. Its file name will always be
`<command_class_name>Brigadier.java`. That means that for `your.package.MyCommand`, the new class name will be
`your.package.MyCommandBrigadier`.

This generated source file will be generated in a separate sources root, which is managed by your build system.
...
Now that we have our Brigadier source file, we can register it in a lifecycle event handler.
