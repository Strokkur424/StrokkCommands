 ...
 * {@link LocationArgumentsCommand} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class LocationArgumentsCommandBrigadier {
  public static final String NAME = "locationargs";
   ...
   * in {@link LocationArgumentsCommand}. You can either retrieve the unregistered node with this method
   * or register it directly with {@link #register(Commands)}.
   */
  public static LiteralCommandNode<CommandSourceStack> create() {
    final LocationArgumentsCommand instance = new LocationArgumentsCommand();

    return Commands.literal(NAME)
      .then(Commands.literal("finepos")
        .then(Commands.argument("finePos", ArgumentTypes.finePosition())
          .executes(ctx -> {
            instance.executes(
              ctx.getSource().getSender(),
              ctx.getArgument("finePos", FinePositionResolver.class).resolve(ctx.getSource())
            );
            return Command.SINGLE_SUCCESS;
          })
        )
        .then(Commands.literal("center")
          .then(Commands.argument("finePos", ArgumentTypes.finePosition(true))
            .executes(ctx -> {
              instance.executesCenter(
                ctx.getSource().getSender(),
                ctx.getArgument("finePos", FinePositionResolver.class).resolve(ctx.getSource())
              );
              return Command.SINGLE_SUCCESS;
      ...
      .then(Commands.literal("rotation")
        .then(Commands.argument("rotation", ArgumentTypes.rotation())
          .executes(ctx -> {
            instance.execute(
              ctx.getSource().getSender(),
              ctx.getArgument("rotation", RotationResolver.class).resolve(ctx.getSource())
            );
            return Command.SINGLE_SUCCESS;
          })
        )
      ...
      .then(Commands.literal("blockpos")
        .then(Commands.argument("blockPos", ArgumentTypes.blockPosition())
          .executes(ctx -> {
            instance.executes(
              ctx.getSource().getSender(),
              ctx.getArgument("blockPos", BlockPositionResolver.class).resolve(ctx.getSource())
            );
            return Command.SINGLE_SUCCESS;
          })
        )
   ...
   */
  private LocationArgumentsCommandBrigadier() throws IllegalAccessException {
