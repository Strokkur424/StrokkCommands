 ...
 * {@link LiteralCombining} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class LiteralCombiningBrigadier {
   ...
   *     LiteralCombiningBrigadier.register(commands);
   * }
   * }</pre>
   */
   ...
   * in {@link LiteralCombining}. You can either retrieve the unregistered node with this method
   * or register it directly with {@link #register(Commands)}.
   */
  public static LiteralCommandNode<CommandSourceStack> create() {
    final LiteralCombining instance = new LiteralCombining();
    final LiteralCombining.SubClass instanceSubClass = new LiteralCombining.SubClass();

    return Commands.literal(NAME)
      .then(Commands.literal("sub")
        .executes(ctx -> {
          instance.normalSub(ctx.getSource().getSender());
        ...
        .then(Commands.literal("class")
          .executes(ctx -> {
            instanceSubClass.execute(ctx.getSource().getSender());
            return Command.SINGLE_SUCCESS;
          })
        )
      )
   ...
   */
  private LiteralCombiningBrigadier() throws IllegalAccessException {
