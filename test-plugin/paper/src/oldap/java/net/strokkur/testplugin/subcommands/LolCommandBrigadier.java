 ...
 * {@link LolCommand} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class LolCommandBrigadier {
  public static final String NAME = "lol";
   ...
   *     LolCommandBrigadier.register(commands);
   * }
   * }</pre>
   */
   ...
   * in {@link LolCommand}. You can either retrieve the unregistered node with this method
   * or register it directly with {@link #register(Commands)}.
   */
  public static LiteralCommandNode<CommandSourceStack> create() {
    return Commands.literal(NAME)
      .then(Commands.argument("player", ArgumentTypes.player())
        .then(Commands.argument("now", StringArgumentType.word())
          .executes(ctx -> {
            final LolCommand executorClass = new LolCommand(
              ctx.getArgument("player", PlayerSelectorArgumentResolver.class).resolve(ctx.getSource()).getFirst(),
              StringArgumentType.getString(ctx, "now")
            );
   ...
   */
  private LolCommandBrigadier() throws IllegalAccessException {
