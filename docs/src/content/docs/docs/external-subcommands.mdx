...
that oftentimes, you end up with a lot of logic for individual root commands. Traditional ways to deal with that
is putting subcommands into its own sites. Such an approach works wonderfully. And StrokkCommands supports it!
...
<Aside type="tip">
  ...
  under [strokk-commands-test-plugin/src/main/java/net/strokkur/testplugin/docs/gamemode](https://github.com/Strokkur424/StrokkCommands/tree/strokk-cmmands-test-plugin/src/main/java/net/strokkur/testplugin/docs/gamemode).
</Aside>
...
Prototype classes support **the full feature set normal commands support**. This means you can even nested
prototype classes into prototype classes! You should just be careful to not accidentally create a
cycle dependency, since that would crash the processor.
