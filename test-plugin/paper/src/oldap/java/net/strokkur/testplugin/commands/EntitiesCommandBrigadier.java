 ...
 * {@link EntitiesCommand} using <a href="https://commands.strokkur.net">StrokkCommands</a>.
 *
 ...
 */
@NullMarked
public final class EntitiesCommandBrigadier {
  public static final String NAME = "entityargs";
   ...
   * in {@link EntitiesCommand}. You can either retrieve the unregistered node with this method
   * or register it directly with {@link #register(Commands)}.
   */
  public static LiteralCommandNode<CommandSourceStack> create() {
    final EntitiesCommand instance = new EntitiesCommand();
    ...
    return Commands.literal(NAME)
        ...
        .then(Commands.argument("entity", ArgumentTypes.entity())
          .executes(ctx -> {
            instance.execute(
              ...
              ctx.getArgument("entity", EntitySelectorArgumentResolver.class).resolve(ctx.getSource()).getFirst()
            );
            return Command.SINGLE_SUCCESS;
          })
        )
      )
      .then(Commands.literal("one-player-profile")
        .then(Commands.argument("profile", ArgumentTypes.playerProfiles())
          .executes(ctx -> {
            instance.execute(
              ctx.getSource().getSender(),
              ctx.getArgument("profile", PlayerProfileListResolver.class).resolve(ctx.getSource()).stream().toList().getFirst()
            );
            return Command.SINGLE_SUCCESS;
          })
        )
      ...
      .then(Commands.literal("playerprofiles")
        .then(Commands.argument("profiles", ArgumentTypes.playerProfiles())
          .executes(ctx -> {
            instance.execute(
              ctx.getSource().getSender(),
              ctx.getArgument("profiles", PlayerProfileListResolver.class).resolve(ctx.getSource()).toArray(PlayerProfile[]::new)
            );
            return Command.SINGLE_SUCCESS;
          })
        )
   ...
   */
  private EntitiesCommandBrigadier() throws IllegalAccessException {
